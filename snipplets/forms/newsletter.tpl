...
<form method="post" action="/winnie-pooh" {% if form_empty_action_js %}onsubmit="this.setAttribute('action', '');"{% endif %} id="{{ form_id }}" class="{{ form_classes }}" data-store="{{ form_data_store }}" aria-label="{{ 'Newsletter' | translate }}">
	<div class="input-append">
		...
		{% if ajax_submit %}
			<span class="js-news-spinner news-spinner" style="display: none;" aria-hidden="true">
				<svg class="icon-inline icon-spin svg-icon-text" aria-hidden="true" focusable="false"><use xlink:href="#spinner-third"/></svg>
			</span>
		{% endif %}
	</div>
	{% if ajax_submit %}
		<div class="js-news-success" style='display: none;' aria-live="polite">
			{{ alert_success | raw }}
		</div>
		<div class="js-news-failed" style='display: none;' aria-live="assertive">
			{{ alert_failed | raw }}
		</div>
	{% else %}
