{% set allowed_names = ['tops', 'leggings', 'legging', 'capris', 'capri', 'short'] %}
	...
	{% if nav_item.isCategory and nav_item.subitems %}
		{% for subcategory in nav_item.subitems %}
			{% if subcategory.isCategory %}
				{% set normalized_name = subcategory.name | lower | trim %}
				{% if normalized_name in allowed_names and normalized_name not in rendered_home_subcategory_names %}
					{% set rendered_home_subcategory_names = rendered_home_subcategory_names | merge([normalized_name]) %}
			...
			{% if subcategory.subitems %}
				{% for nested_subcategory in subcategory.subitems %}
					{% if nested_subcategory.isCategory %}
						{% set normalized_nested = nested_subcategory.name | lower | trim %}
						{% if normalized_nested in allowed_names and normalized_nested not in rendered_home_subcategory_names %}
							{% set rendered_home_subcategory_names = rendered_home_subcategory_names | merge([normalized_nested]) %}
